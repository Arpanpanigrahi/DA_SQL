...
with ABC as(
select *, DENSE_RANK() over (partition by country order by sale) as Rank from CNTRY
)
select * from abc
where rank = 3
...
select *, LAG(spent,1) over(partition by [Name], Restraunt order by [Name], Restraunt asc) as PREVIOUS_SPENT
from HOTEL
...
--Using WITH Clause and Windows Function Not working
select abc as (
select *, LAG(dov,1) over(partition by [Name], Restraunt order by [Name], Restraunt) as Prev_Day
from HOTEL
)
